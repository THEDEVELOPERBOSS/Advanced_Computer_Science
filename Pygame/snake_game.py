...
import pygame 
import random
# tutorial used https://thepythoncode.com/article/make-a-snake-game-with-pygame-in-python
# set up some initial parameters
WIDTH, HEIGHT = 600, 600
BLOCK_SIZE = 20

pygame.font.init()
score_font = pygame.font.SysFont("consolas", 20) # or any other font
score = 0
...
# snake and food intitialization 
snake_pos = [[WIDTH//2, HEIGHT//2]]
snake_speed = [0, BLOCK_SIZE]

teleport_walls = True # set this to True to enable wall teleporting. If false game will end if wall is hit

def generate_food():
    while True: # makes sure food does not appear where snake is 
...
def draw_objects():
    ...
    win.blit(score_text, (10, 10)) # draws the score in top left corner
...
def update_snake():
    ...
    if teleport_walls:
        ...
        if new_head == food_pos:
            food_pos = generate_food() # generat new food
...
def run():
    ...
    while running:
        ...
        draw_objects()
        pygame.display.update()
        clock.tick(15)  # limit the frame rate to 15 FPS(HIGHER FPS FASTER MOVEMENT)
